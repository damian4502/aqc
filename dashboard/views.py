...
def room_live_data(request, room_id):
    ...
    # Latest measurements for this room
    from measurements.models import Measurement
...
def apply_dark_theme(fig, animate=True):
    """Dark theme + neon effects with safety for different graph types"""
    
    # Base dark theme (works for all graph types)
    fig.update_layout(
    ...
    # Neon + animations only for line/scatter graphs (not for imshow/heatmap)
    ...
    # Special handling for heatmap / imshow (correlation)
...
def export_room_csv(request, room_id):
    ...
    # Prefer new spike_factor; fall back to legacy ignore_spikes
...
def custom_dashboard(request, dashboard_id):
    ...
    if not dashboard:
        dashboard = Dashboard.objects.create(name="My dashboard", is_default=True)
...
def room_detail(request, room_id):
    ...
    context['spike_factor'] = spike_factor
    context['ignore_spikes'] = spike_factor > 0  # backward compat for templates
    ...
    # === Save / read resampling settings from session ===
    if request.method == 'GET':
        # If user sends new values → save to session
    ...
    if view_type == 'trend':
        ...
        if resampled.empty or len(resampled.columns) < 2:
            ...
            fig = px.imshow(
                ...
                title=f'Parameter correlation - {room.name} (interval: {interval_minutes} min)'
            )
        ...
        fig = px.line(hourly, x='hour', y='value', color='parameter',
                     title='Daily pattern (average by hour)', markers=True)
        ...
        fig_heatmap = px.imshow(
            ...
            title='Weekly heatmap pattern (average values)'
        )
...
def pressure_calibration_view(request):
    """Page for pressure sensor calibration (compare 2 sensors + MQTT offset)."""
...
def set_pressure_offset_api(request):
    """
    Update pressure_offsets in offsets.json.
    Newly measured delta is ADDED to the existing offset in the JSON,
    because the sensor already publishes the value WITH the current offset applied.
    """
    try:
        payload = json.loads(request.body)
        sensor_id = int(payload.get("sensor_id"))
        delta = float(payload.get("offset"))          # this is avgDelta from calibration (ref - cal)
    ...
    if not offsets_path.exists():
        return JsonResponse({"success": False, "error": f"offsets.json not found: {offsets_path}"}, status=500)
    ...
    # === KEY: add existing offset + newly measured delta ===
    ...
    return JsonResponse({
        ...
        "message": f"For '{key}': {current_offset:.2f} + {delta:.2f} = {new_total_offset:.2f} Pa"
    })
    
    
import pandas as pd
from django.utils import timezone
def resample_measurements(df, interval_minutes=15, fill_method='ffill', ignore_spikes=False, spike_factor=None):
    """Resample measurements and optionally remove outliers using IQR method.

    spike_factor: IQR multiplier for outlier bounds. Higher = milder filtering.
      0 or None + ignore_spikes=False => no filtering
      Typical values: 3.5 (mild), 2.5 (medium), 1.5 (aggressive)
    ignore_spikes: legacy bool; if True and spike_factor is None, uses 2.5
    """
    if df.empty:
        return pd.DataFrame()
    ...
    # Resolve spike_factor (IQR multiplier). Higher value = less aggressive filtering.
    if spike_factor is None:
        spike_factor = 2.5 if ignore_spikes else 0.0
    try:
        spike_factor = float(spike_factor)
    except (TypeError, ValueError):
        spike_factor = 0.0

    # Outlier removal via IQR method; extreme values replaced by rolling median
    if spike_factor > 0 and not resampled.empty:
        for col in resampled.columns:
            series = resampled[col]
            Q1 = series.quantile(0.25)
            Q3 = series.quantile(0.75)
            IQR = Q3 - Q1
            lower_bound = Q1 - spike_factor * IQR
            upper_bound = Q3 + spike_factor * IQR
...
def parse_spike_factor(value, default=0.0):
    """Parse spike_factor from query/session. Returns IQR multiplier (0 = off).

    Accepts numeric strings (e.g. '1.5', '2.5', '3.5') or legacy bool-like
    values (on/true => 2.5).
    """
    if value is None:
    ...
    try:
        ...
        if parse_bool_flag(value, default=False):
            return 2.5
        return 0.0

def chart_data(request):
    from django.utils.dateparse import parse_datetime
    """Returns Plotly JSON for dynamic loading in JS."""
    room = None
    parameter = None
    ...
    # --- Parse parameters (same as now) ---
    start_str = request.GET.get('start')
    end_str = request.GET.get('end')
    interval = int(request.GET.get('interval', 60))  # minutes
    ...
    context['fill_method'] = fill_method
    context['spike_factor'] = spike_factor
    context['ignore_spikes'] = spike_factor > 0

    resampled = resample_measurements(df, interval_minutes, fill_method, spike_factor=spike_factor)
    if not resampled.empty:
        fig = px.line(resampled, x=resampled.index, y=resampled.columns,
                     title=f'Time trend',
                     height=700)
    ...
    # ... your code for df and traces ...

    #fig = go.Figure()
    # ... add traces with visible='legendonly' where you want ...

    # --- Events as vlines + annotations ---
    events = Event.objects.filter(
        timestamp__gte=start,
        timestamp__lte=end
    ).distinct()
...
def room_graph_fragment(request, room_id):
    """Returns ONLY the graph for HTMX (fragment)"""
    room = get_object_or_404(Room, id=room_id)
    view_type = request.GET.get('view', 'trend')
    all_data = request.GET.get('all') == 'true'
    ...
    if not measurements.exists():
        return HttpResponse('<div class="text-center py-20 text-slate-400">No data for the selected period.</div>')
    ...
    if view_type == 'trend':
        ...
        for i in range(len(params)):
            for j in range(i+1, len(params)):
                ...
                if corr > 0.3:  # show only significant correlations
        ...
        fig.update_layout(
            title='Correlation network between parameters',
            height=700,
            showlegend=False,
    ...
    return HttpResponse('<div class="text-center py-20 text-slate-400">No graphical data.</div>')

def create_aqi_gauge(aqi_value):
    """Smaller AQI Gauge with color coding"""
    if not aqi_value:
        aqi_value = 0
...
def dashboard_overview(request):
    ...
    for room in rooms:
        # Latest measurements (we still show all parameters in the table)
        ...
        """
        # Mini graph - only AQI for the last 24 hours
        mini_fig = None
        aqi_measurements = Measurement.objects.filter(
            sensor__room=room,
            parameter__name__iexact="AQI",   # look for parameter named AQI (case-insensitive)
...
def dashboard(request):
    # Get last 7 days of data (you can change this)
    ...
    # Main graph: all parameters over time
    fig_main = px.line(
        df, 
        x='timestamp', 
        y='value', 
        color='parameter',
        facet_col='room',
        title='Measurements by rooms and parameters (last 7 days)',
...
def parameter_detail(request, parameter_id):
    ...
    context['fill_method'] = fill_method
    context['spike_factor'] = spike_factor
    context['ignore_spikes'] = spike_factor > 0  # backward compat
    ...
    if measurements.exists():
        ...
        if view_type == 'trend':
            ...
            fig = px.line(resampled, x=resampled.index, y=resampled.columns,
                         title=f'Time trend - {parameter.name}',
                         height=700)
            fig = apply_dark_theme(fig)
            ...
            if resampled.empty or len(resampled.columns) < 2:
                ...
                fig = px.imshow(
                    ...
                    aspect="auto",
                    color_continuous_scale='RdBu_r',
                    title=f'Parameter correlation - {parameter.name} (interval: {interval_minutes} min)'
                )
                fig.update_layout(height=650)
                fig = apply_dark_theme(fig)
                context['fig'] = fig.to_html(full_html=False, include_plotlyjs='cdn')
            ...
            fig = px.line(hourly, x='hour', y='value', color='room',
                         title=f'Daily pattern for {parameter.name}', markers=True)
            fig.update_layout(xaxis=dict(tickmode='linear', dtick=1))
            fig = apply_dark_theme(fig)
            context['fig'] = fig.to_html(full_html=False, include_plotlyjs='cdn')
            ...
            fig = px.line(weekly, x='dayofweek', y='value', color='room',
                         title=f'Weekly pattern for {parameter.name}',
                         category_orders={"dayofweek": ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]})
            fig = apply_dark_theme(fig)
            context['fig'] = fig.to_html(full_html=False, include_plotlyjs='cdn')
...
def trends_view(request):
    ...
    for param in parameters:
        ...
        for room in Room.objects.all():
            ...
            # === SMART COLORING based on higher_is_worse ===
            if param.higher_is_worse:
                # Higher = worse → negative slope is good
                trend_color = 'text-emerald-400' if slope <= 0 else 'text-red-400'
            else:
                # Higher = better (e.g. temperature) → positive slope can be good
        ...
        # Sorting: for "bad" parameters we want the smallest slope on top
        if param.higher_is_worse:
            rooms_data.sort(key=lambda x: x['slope'])           # smaller (more negative) = better
        else:
...
def debug_mqtt(request):
    """Debug page for MQTT – subscribe to topics and send messages."""
